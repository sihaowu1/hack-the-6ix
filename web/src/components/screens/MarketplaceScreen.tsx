  ...
  return (
    <main className="mx-auto w-full max-w-[1200px] overflow-y-auto p-6">
      ...
      {loading ? (
        ...
        <p className="text-[13px] text-text-dim">No published scenes yet. Be the first!</p>
      ) : (
        <>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
            {items.map((item) => (
              <Link
                ...
                className="flex flex-col gap-2 rounded-lg border border-border bg-bg-raised p-4 text-inherit no-underline transition-colors hover:border-accent"
              >
                <h3 className="m-0 text-[16px] font-semibold text-text">{item.title}</h3>
                <p className="m-0 flex-1 text-[13px] text-text-dim">{item.description || 'No description'}</p>
