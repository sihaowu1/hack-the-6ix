...
export function MarketplaceDetailScreen() {
  ...
  if (loading) return <main className={containerClassName}><p className="text-[13px] text-text-dim">Loading…</p></main>;
  if (error) return <main className={containerClassName}><p className="text-[13px] text-red-400">{error}</p></main>;
  if (!item) return <main className={containerClassName}><p className="text-[13px] text-text-dim">Not found.</p></main>;
    ...
    <main className={containerClassName}>
      <div className="grid min-h-[300px] grid-cols-2 gap-5">
        <div className="flex flex-col gap-2">
          ...
          <p className="m-0 text-[14px] text-text-dim">{item.description}</p>
          <div className="mt-auto flex items-center gap-2 text-[12px] text-text-dim">
      ...
      <div className="flex flex-col">
        <div className="flex gap-0">
          <button
            ...
            className={
              tab === 'threejs'
                ? 'border border-border bg-bg-raised px-4 py-2 text-[13px] font-semibold text-text'
                : 'border border-border bg-bg-panel px-4 py-2 text-[13px] text-text-dim'
            }
          ...
          {item.blenderCode && (
            <button
              ...
              className={
                tab === 'blender'
                  ? 'border border-border border-b-0 bg-bg-raised px-4 py-2 text-[13px] font-semibold text-text'
                  : 'border border-border bg-bg-panel px-4 py-2 text-[13px] text-text-dim'
              }
        ...
        <pre className="m-0 max-h-[400px] overflow-auto rounded-b-lg border border-border bg-bg-raised p-4 text-[12px] leading-relaxed">
